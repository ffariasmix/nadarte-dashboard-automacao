...
const r = motor({ freq, crm, aniv, cfg: { cap: Number(process.env.TETO_UNIDADE || 120) } });
...
console.error(`[motor] semana ${semana} (prazo ${PRAZO}): ${rows.length} · alerta ${r.alerta.length} · ativa ${r.ativa.length} · reserva ${r.reserva.length} · relac ${r.relacionamento.length} · descartadas ${r.descartadas}`);
